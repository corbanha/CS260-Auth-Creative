...
function addPiece() {
    ...
    //check and see if the blocks have gotten to the top yet and end
    var piecesToTheTop = false;
    for (var i = 0; i < placedPieces.length; i++) {
        if (placedPieces[i].ypos <= 1 && placedPieces[i].xpos < GAME_BOARD_WIDTH / 2 + 2 &&
            placedPieces[i].xpos > GAME_BOARD_WIDTH / 2 - 2) {
            piecesToTheTop = true;
            break;
        }
    }
    ...
    if (piecesToTheTop) {
        ...
        alert("You got a score of " + score + ". Great job!");
        ...
        $.ajax({
            ...
            success: console.log("Successfully sent off " + score + " to the server!"),
            dataType: "text/json"
        });
